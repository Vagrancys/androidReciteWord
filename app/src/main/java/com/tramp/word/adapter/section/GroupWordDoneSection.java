...
public class GroupWordDoneSection extends StatelessSection{
    private Context mContext;
    private Activity mActivity;
    private ArrayList<GroupMainRankInfo.Rank.Item> Ranks;
    private GroupMainInterFace mFace;
    public GroupWordDoneSection(Context context,Activity activity,ArrayList<GroupMainRankInfo.Rank.Item> ranks){
        super(R.layout.item_empty,R.layout.item_group_rank_foot,R.layout.item_group_rank_item);
        mContext=context;
        mActivity=activity;
        Ranks=ranks;
    ...
    public void onBindItemViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        mHolder.MemberNumber.setVisibility(View.GONE);
        mHolder.MemberImg.setVisibility(View.GONE);
        if(position==0){
            mHolder.MemberImg.setVisibility(View.VISIBLE);
            mHolder.MemberImg.setImageResource(R.drawable.group_top_1);
        }else if(position==1){
            mHolder.MemberImg.setVisibility(View.VISIBLE);
            mHolder.MemberImg.setImageResource(R.drawable.group_top_2);
        }else if(position==2){
            mHolder.MemberImg.setVisibility(View.VISIBLE);
            mHolder.MemberImg.setImageResource(R.drawable.group_top_3);
        }else{
            mHolder.MemberNumber.setVisibility(View.VISIBLE);
            mHolder.MemberNumber.setText(position);
        }
    ...
    public void onBindFooterViewHolder(RecyclerView.ViewHolder holder) {
        ...
        if(Ranks.size()==1){
            mFooter.GroupRankDone.setVisibility(View.GONE);
            mFooter.GroupRankShare.setVisibility(View.VISIBLE);
        }else{
            mFooter.GroupRankDone.setVisibility(View.VISIBLE);
        ...
        mFooter.ItemGroupRankNumber.setText("以上"+Ranks.size()+"位同学已完成目标");
    }
    ...
    public class FooterViewHolder extends RecyclerView.ViewHolder{
        @BindView(R.id.item_group_rank_number)
        TextView ItemGroupRankNumber;
        @BindView(R.id.group_rank_done)
        LinearLayout GroupRankDone;
