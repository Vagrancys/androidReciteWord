 ...
 * version:1.0
 */
public class SpellContentAdapter extends AbsRecyclerViewAdapter {
    private ArrayList<DefaultPinyinInfo> mArray;
    private int mStatus;
    private int mSuccess;
    public SpellContentAdapter(RecyclerView recyclerView, ArrayList<DefaultPinyinInfo> array, int status, int success){
        super(recyclerView);
        mArray=array;
        mStatus=status;
        mSuccess=success;
    ...
    public void onBindViewHolder(ClickableViewHolder holder, int position) {
        ...
        if(position==mArray.size()-1){
            mHolder.mItemPinContent.setText(getContext().getResources().getString(R.string.pin_context_text));
        }
